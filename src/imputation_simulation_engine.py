...
def main(args):
	...
	for tp in range(t):
			...
			f = open(f'{prefix}sphyr_{t}_{prop}_{sd}.txt', "w")
			...
			np.savetxt(f'{prefix}scite_{t}_{prop}_{sd}.csv', char_mat.T, delimiter=' ', fmt='%d')
			...
			df.to_csv(f'{prefix}phyllochron_{t}_{prop}_{sd}_character_matrix.csv')
